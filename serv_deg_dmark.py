...
#Agrego pin por dentro del tunel para el caso que de bien por fuera
# Analizar el ping
ping_out = net_connect.send_command('ping ' + ipaddr_nbma + ' repeat 100 timeout 1')
print(ping_out)
net_connect.disconnect()
ping_in = net_connect.send_command('ping ' + '192.168.5.142' + ' repeat 100 timeout 1')
print(ping_in)
net_connect.disconnect()
...
# Verificar contadores y negociación de la interfaz conectada al cliente
if dev_vers in 'C8':
    net_connect = ConnectHandler(device_type='cisco_ios_telnet', ip='192.168.5.142', username='ign', password='auto')
...
#Agrego pin por dentro del tunel para el caso que de bien por fuera
#Falta:
#cuando se verificar contadores y negociación de la interfaz conectada al proveedor tambien se deberia vericar para el lado del cliente.
#Ping sin y con mtu 
#trace route desde el concentrador al PE
#Dudas del codigo:
#if dev_vers in 'C8':
